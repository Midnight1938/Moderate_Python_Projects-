...
float fbm(vec2 p) { // fractal brownian motion. ie noise with octaves. what roughens the terrain
    float res = 0.0;
    float amp = 0.5;
    float freq = 1.95;
    for( int i = 0; i < NUM_OCTAVES; i++) {
        res += amp * noise(p);
        amp *= 0.5;
        p = p * freq * rot(PI / 4.0) - res * 0.4; // Diversify terrain to not repeat
    }
    return res;
}
...
float getWater(vec3 p) {
    ...
    return d; // Cut the terrain in a wave
}
...
float map(vec3 p) {
    float d = 0.0;
    d += getTerrain(p);
    return min(d, getWater(p) + d); // d textures the water
}
